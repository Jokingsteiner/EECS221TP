...
@WebServlet("/Var3Control")

public class Var3Control extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // global s and t are still not set
            ...
            ArrayList<LinkedList<Integer>> hlPaths = edp.findMinCostMaxEdjPaths(null, src, dest);
            GraphVisualizer gv = new GraphVisualizer();
            Graph<Integer, CustomEdge> g = gv.createGraph(edp.getRawGraph());
            gv.colorEdges(g, hlPaths);
            String baseGraphPath = request.getServletContext().getAttribute("FILES_DIR")+ File.separator + "variation3.png";
            gv.saveGraph(null, baseGraphPath);
            request.setAttribute("error",false);
            request.setAttribute("message", "Variation3 result generated.");
            request.setAttribute("var3_graph", "variation3.png");
            request.setAttribute("path_set", hlPaths);
            request.setAttribute("path_cost", edp.getPathSetCost(g, hlPaths));
            request.getRequestDispatcher("view/variation3.jsp").forward(request, response);
        }
    }
